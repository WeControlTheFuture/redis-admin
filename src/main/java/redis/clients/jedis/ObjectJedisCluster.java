...
public class ObjectJedisCluster extends BinaryJedisCluster {
	...
	public ObjectJedisCluster(Set<HostAndPort> jedisClusterNode, int connectionTimeout, int soTimeout,
	...
	public String set(final String key, final Object value) {
		return new JedisClusterCommand<String>(connectionHandler, maxRedirections) {
			...
			public String execute(Jedis connection) {
				...
				connection.getClient().set(SafeEncoderMore.encode(key), SafeEncoderMore.encode(value));
	...
	public Long setnx(final String key, final Object value) {
		return new JedisClusterCommand<Long>(connectionHandler, maxRedirections) {
			...
			public Long execute(Jedis connection) {
				connection.checkIsInMultiOrPipeline();
				connection.getClient().setnx(SafeEncoderMore.encode(key), SafeEncoderMore.encode(value));
	...
	public Long hsetnx(final String key, final String field, final Object value) {
		return new JedisClusterCommand<Long>(connectionHandler, maxRedirections) {
			...
			public Long execute(Jedis connection) {
				connection.checkIsInMultiOrPipeline();
				connection.getClient().hsetnx(SafeEncoderMore.encode(key), SafeEncoderMore.encode(field),
						SafeEncoderMore.encode(value));
				return connection.getClient().getIntegerReply();
			}
		}.run(key);
	}
			...
			public Object execute(Jedis connection) {
				try {
					...
					if (e.getCause() instanceof InvalidClassException) {
						// here we swallow this exception because Object put
						// into redis may have different serializeId
						// if we suffer this situation, we just return null
						// instead
						// TODO: add some log ......
					} else
						throw e;
	...
	public String setex(final String key, final int seconds, final Object value) {
		return new JedisClusterCommand<String>(connectionHandler, maxRedirections) {
			...
			public String execute(Jedis connection) {
				connection.checkIsInMultiOrPipeline();
				connection.getClient().setex(SafeEncoderMore.encode(key), seconds, SafeEncoderMore.encode(value));
				return connection.getClient().getStatusCodeReply();
			}
		}.run(key);
	}
